...
*
*/
A2BMenu::A2BMenu(WINDOW* parent, std::vector<std::string> setA, std::string setA_header, std::vector<std::string> setB, std::string setB_header, std::vector<int> &result, int x_offset, int y_offset)
	: m_setA_header(setA_header), m_setB_header(setB_header), m_setA_size(setA.size()), m_setB_size(setB.size()), m_result(&result)
{
	...
	int width = std::max(m_longest_in_setA, m_setA_header.size()) + std::max(m_longest_in_setB, m_setB_header.size()) + 8;
	std::string space(width - 4, ' ');

	m_Menu = subwin(parent, m_setA_size + 4, width, x_offset, y_offset);
	box(m_Menu, 0, 0);

	mvwprintw(m_Menu, 1, 2, m_setA_header.c_str());										//print setA header in menu subwindow
	mvwprintw(m_Menu, 1, width - (m_setB_header.size() + 2), m_setB_header.c_str());	//print setB header in submenu
	...
	for (size_t i = 0; i < m_setA.size(); i++)
	{
		...
		if (i == 0) 
		{
			...
			wprintw(m_Menu, space.c_str());		//highlighted whitespace
			wmove(m_Menu, i + 3, 2);
		}
		...
		if (i == 0)								//print arrows
		{
			mvwprintw(m_Menu, 3, (width - 3), ">");
			mvwprintw(m_Menu, 3, (width - (4 + m_longest_in_setB)), "<");
		}
		if (i == 0) wattroff(m_Menu, A_STANDOUT);
	}
	wrefresh(m_Menu);
...
int A2BMenu::select()
{
	int ch, counter = 1000092, selected = 0, width = std::max(m_longest_in_setA, m_setA_header.size()) + std::max(m_longest_in_setB, m_setB_header.size()) + 8;
	std::string space(width - 4, ' ');
	...
	while (((ch = getch()) != KEY_ENTER && ch != 10) || !valid_state)
	{
		...
		for (size_t i = 0; i < m_setA_size; i++)
		{
			...
			{
				wattron(m_Menu, A_STANDOUT);
				wprintw(m_Menu, space.c_str());
				mvwprintw(m_Menu, (i+3), (width - 3), ">");
				mvwprintw(m_Menu, (i+3), (width - (4 + m_longest_in_setB)), "<");
				wmove(m_Menu, i + 3, 2);
			}
			wprintw(m_Menu, (m_setA[i].name).c_str());
			if (m_setA[i].mapped_to >= 0 && m_setA[i].mapped_to < static_cast<int>(m_setB.size()))
				mvwprintw(m_Menu, i + 3, width - (m_setB[m_setA[i].mapped_to].name.size() + 3), m_setB[m_setA[i].mapped_to].name.c_str()); //print setB item for given setA item
			if (i == selected)
